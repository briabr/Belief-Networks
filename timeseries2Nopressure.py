...
for eps in eps_values:
    ...
    dfs[eps] = pd.read_csv(
        f"simOut/detailed/sim_init_w{init_w:.2f}_beta3.00_eps{eps:.2f}_ext_strength{s}_seed{seed}_detailed.csv"
    )
df = dfs[0.0]
print("epsilon 0:", dfs[0.0].shape)
print("epsilon 1:", dfs[1.0].shape)
# %%
...
beliefs_eps1 = dfs[1.0][["id", "t"] + ["0"]].pivot_table(
...
for adaptive, w0 in zip([True], [0.2]):
    ...
    axs["t"].set_title(
f"example simulation with static belief networks",        fontsize=smallfs,
    )
fname = f"2026-04_figs/fig2_static"
